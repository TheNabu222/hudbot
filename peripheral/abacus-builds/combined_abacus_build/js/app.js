...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  Assets.init();
  Canvas.init();
  if (typeof DeviceFrame !== 'undefined') DeviceFrame.init();
  if (typeof HudStudio !== 'undefined') HudStudio.init();
  Layers.init();
  Properties.init();
  Hitbox.init();
  ...
  RPGSystems.init();
  if (typeof WorldWorkspace !== 'undefined') WorldWorkspace.init();
    ...
    tabs.querySelectorAll('.panel-tab').forEach(tab => {
      tab.addEventListener('click', () => {
        ...
        if (panel.id === 'left-panel') {
          panel.classList.toggle('asset-browser-open', tabName === 'assets');
          panel.classList.toggle('hud-studio-open', tabName === 'hud');
        }

        if (panel.id === 'left-panel' && typeof WorldWorkspace !== 'undefined') {
          if (tabName === 'world') WorldWorkspace.open();
          else WorldWorkspace.close();
        }
        ...
        if (tabName === 'world' && typeof WorldWorkspace !== 'undefined') {
        ...
        if (tabName === 'hud' && typeof HudStudio !== 'undefined') {
          HudStudio.render();
        }
        if (tabName === 'flags') {
  ...
  document.getElementById('btn-settings').addEventListener('click', () => {
    ...
    leftPanel.classList.remove('asset-browser-open');
    leftPanel.classList.remove('hud-studio-open');
  ...
  // A same-site project link is useful for large saves that exceed localStorage.
  const linkedProjectPath = new URLSearchParams(window.location.search).get('project');
  const loadedLinkedProject = linkedProjectPath
    ? await Project.loadFromUrl(linkedProjectPath)
    : false;

  // Try the linked project, then autosave, else create a fresh project.
  if (!loadedLinkedProject && (!State.loadAutoSave() || !State.project.scenes.length)) {
  ...
  if (!State.project.startSceneId && State.project.scenes.length) {
    State.project.startSceneId = State.project.activeSceneId || State.project.scenes[0].id;
  }
  ...
  console.log('Anzu Game Studio loaded. Abacus + Cavebot unified workspace ready.');
});
