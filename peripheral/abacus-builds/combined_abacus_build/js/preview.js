...
const Preview = {
  ...
  enter() {
    ...
    // Phase 3D: Reset clickbox state
    this._clickboxFiredOnce = {};
  ...
  exit() {
    ...
    // Phase 3D: Clean up clickbox state
    this._clickboxFiredOnce = {};
    this._hideClickboxTooltip && this._hideClickboxTooltip();
  },
  ...
  _renderScene(scene) {
    ...
    for (const obj of sorted) {
      ...
      // Phase 3D: Check if object was hidden by toggle-object action
      if (this.objectStates[obj.id]?.hidden) continue;
    ...
    // Render clickable zones (Phase 3D)
    if (!this._clickboxFiredOnce) this._clickboxFiredOnce = {};
    for (const hb of scene.hitboxes) {
      if (typeof Hitbox !== 'undefined') Hitbox.migrateBox(hb);
      this._renderClickbox(canvas, hb);
    }
    ...
    if (typeof HudRuntime !== 'undefined') HudRuntime.renderOpenMenus(canvas, this);

    const previewDisplay = (typeof DeviceFrame !== 'undefined')
      ? DeviceFrame.wrapPreview(canvas, this)
      : canvas;
    this.previewStage.appendChild(previewDisplay);
  ...
  // ===== Phase 3D: Clickbox Rendering & Actions =====
  _renderClickbox(canvas, hb) {
    const conditionResult = this._evaluateClickboxConditions(hb);
    if (!conditionResult && hb.hideWhenConditionFails) return; // Completely hidden

    const zone = document.createElement('div');
    zone.className = 'clickbox-zone';
    if (hb.hoverEffect && hb.hoverEffect !== 'none') {
      zone.classList.add('fx-' + hb.hoverEffect);
    }
    ...
    zone.style.cssText = `
      ...
      cursor: ${hb.cursor || 'pointer'};
      z-index: 9000;
    `;
    zone.dataset.clickboxId = hb.id;

    // Hover tooltip
    if (hb.hoverTooltip) {
      zone.addEventListener('mouseenter', (e) => this._showClickboxTooltip(canvas, hb, e));
      zone.addEventListener('mouseleave', () => this._hideClickboxTooltip(canvas));
    }

    // Click handler
    zone.addEventListener('click', (e) => {
      e.stopPropagation();
      this._hideClickboxTooltip(canvas);
      if (!this._evaluateClickboxConditions(hb)) return; // still gated at click time
      if (hb.oneShot && this._clickboxFiredOnce[hb.id]) return;
      this._clickboxFiredOnce[hb.id] = true;
      this._runClickboxAction(hb);
    });

    canvas.appendChild(zone);
  },

  _showClickboxTooltip(canvas, hb, e) {
    this._hideClickboxTooltip(canvas);
    const tip = document.createElement('div');
    tip.className = 'clickbox-tooltip';
    tip.textContent = hb.hoverTooltip;
    const rect = e.target.getBoundingClientRect();
    const canvasRect = canvas.getBoundingClientRect();
    tip.style.left = (rect.left - canvasRect.left + rect.width / 2 - 50) + 'px';
    tip.style.top  = (rect.top  - canvasRect.top - 28) + 'px';
    canvas.appendChild(tip);
  },

  _hideClickboxTooltip(canvas) {
    canvas?.querySelectorAll('.clickbox-tooltip').forEach(t => t.remove());
  },

  _evaluateClickboxConditions(hb) {
    const conds = hb.conditions || [];
    if (!conds.length) return true;
    const mode = hb.conditionMode || 'all';
    const evals = conds.map(c => this._evalSingleClickboxCondition(c));
    return mode === 'any' ? evals.some(Boolean) : evals.every(Boolean);
  },

  _evalSingleClickboxCondition(c) {
    if (!c || !c.type) return true;
    switch (c.type) {
      case 'flag':
        return GameFlags.evaluateCondition(c, this.runtimeFlags);
      case 'inventory': {
        const has = this.hasItem(c.itemId);
        return (c.operator || 'has') === 'not' ? !has : has;
      }
      case 'reputation': {
      ...
      case 'skill': {
        ...
        return val >= Number(c.value || 0);
      }
      case 'time': {
        const t = this.rpgState?.time || { hour: 0, day: 0 };
  ...
  _runClickboxAction(hb) {
    const t = hb.actionType || hb.action || 'none';
    switch (t) {
      case 'scene-change':
        if (hb.targetSceneId) this.transitionTo(hb.targetSceneId);
        break;
      case 'start-dialogue':
        if (hb.dialogueTreeId) this.runDialogueTree(hb.dialogueTreeId);
        break;
      case 'toggle-object':
        this._clickboxToggleObject(hb.toggleObjectId, hb.toggleMode);
        break;
      case 'add-item':
        if (hb.itemId) {
          const n = Math.max(1, hb.itemCount || 1);
          for (let i = 0; i < n; i++) this.giveItem(hb.itemId);
        }
        break;
      case 'remove-item':
        if (hb.itemId) this.removeItem(hb.itemId, Math.max(1, hb.itemCount || 1));
        break;
      case 'set-flag':
        if (hb.setFlag && hb.setFlag.flag) {
          GameFlags.applyAction(hb.setFlag, this.runtimeFlags);
          this._showPreviewToast('🚩 ' + hb.setFlag.flag + ' updated');
        }
        break;
      case 'play-sound':
        this._clickboxPlaySound(hb.soundAssetId, hb.soundVolume);
        break;
      case 'multiple':
        (hb.actionChain || []).forEach(step => this._runChainStep(step));
        break;
      case 'none':
      default:
        break;
  ...
  _runChainStep(step) {
    ...
    switch (step.type) {
      case 'scene-change':
        if (step.targetSceneId) this.transitionTo(step.targetSceneId);
        break;
      case 'start-dialogue':
        if (step.dialogueTreeId) this.runDialogueTree(step.dialogueTreeId);
        break;
      case 'toggle-object':
        this._clickboxToggleObject(step.toggleObjectId, step.toggleMode);
        break;
      case 'add-item':
        if (step.itemId) for (let i = 0; i < Math.max(1, step.itemCount || 1); i++) this.giveItem(step.itemId);
        break;
      case 'remove-item':
        if (step.itemId) this.removeItem(step.itemId, Math.max(1, step.itemCount || 1));
        break;
      case 'set-flag':
        if (step.setFlag && step.setFlag.flag) GameFlags.applyAction(step.setFlag, this.runtimeFlags);
        break;
      case 'play-sound':
        this._clickboxPlaySound(step.soundAssetId, step.soundVolume);
        break;
    }
  },

  _clickboxToggleObject(objId, mode) {
    if (!objId) return;
    if (!this.objectStates[objId]) this.objectStates[objId] = {};
    const cur = this.objectStates[objId].hidden;
    const m = mode || 'toggle';
    if (m === 'show') this.objectStates[objId].hidden = false;
    else if (m === 'hide') this.objectStates[objId].hidden = true;
    else this.objectStates[objId].hidden = !cur;
    // Re-render
    this.renderPreviewScene(this.currentSceneId, 'none');
  },

  _clickboxPlaySound(assetId, volume) {
    if (!assetId) return;
    const asset = State.project.assets.find(a => a.id === assetId);
    if (!asset) return;
    try {
      const audio = new Audio(asset.dataURL);
      audio.volume = volume == null ? 0.8 : Math.max(0, Math.min(1, volume));
      audio.play();
    } catch (e) {
      console.warn('Clickbox sound failed:', e);
    }
  },
  ...
  _renderRPGHUD(canvas) {
    ...
    if (State.project.globalSettings && typeof HudRuntime !== 'undefined') {
      HudRuntime.renderBuiltInHud(canvas, this);
      return;
    }
    if (!this.rpgState) return;
  ...
  _applyConsequences(consequences = []) {
    ...
    for (const r of consequences) {
      ...
      if (type === 'flag') {
        ...
        if (r.action === 'advance') QuestTracker.advanceMilestone(this.rpgState.quests, r.questId, r.milestoneId, this.runtimeFlags, this.rpgState.reputation);
      } else if (type === 'needs') {
  ...
  handleClick(obj) {
    ...
    if (obj.questAction && obj.questAction.questId && this.rpgState) {
      if (obj.questAction.milestoneId) {
        QuestTracker.advanceMilestone(this.rpgState.quests, obj.questAction.questId, obj.questAction.milestoneId, this.runtimeFlags, this.rpgState.reputation);
    ...
    if (availableChoices.length > 0) {
      dlg.querySelectorAll('.dlg-choice-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
          ...
          // Functional MVP social branch flags
          if ((choice.socialTag === 'boundary-setting' || choice.socialTag === 'conflict-resolving') && this.rpgState) {
            const repNpcId = node.npcId || choice.npcId || '';
            if (repNpcId) {
              Reputation.applyRepChange(this.rpgState.reputation, repNpcId, 'Friendship', Number(choice.socialRepDelta || 0));
              this._showPreviewToast(`${choice.socialTag === 'boundary-setting' ? '🧭 Boundary' : '🤝 Conflict'} choice impacted relationship`);
            }
          }
          if (choice.questBranch && this.rpgState) {
            const [qid, mid] = String(choice.questBranch).split('::');
            if (qid && mid) QuestTracker.advanceMilestone(this.rpgState.quests, qid, mid, this.runtimeFlags, this.rpgState.reputation);
          }
  ...
  _renderInventoryBar(canvas) {
    ...
    const hudSettings = State.project.globalSettings;
    if (hudSettings?.hideAllDefaultHud || hudSettings?.hideDefaultInventoryBtn) return;
  ...
  _renderSaveLoadButtons(canvas) {
    const hudSettings = State.project.globalSettings;
    if (hudSettings?.hideAllDefaultHud || hudSettings?.hideDefaultSettingsBtn) return;
