...
const AssetManager = {
  ...
  render() {
    ...
    if (typeof HelpSystem !== 'undefined') HelpSystem.attachContextualHelp();
  },
  ...
  _renderMetadata(container) {
    ...
    if (!this.metadataAssetId) {
      ...
      if (this.selectedAssets.size > 0) {
        ...
        el.innerHTML = '<div class="empty-state"><p>Double-click an asset or select one to view details &amp; info</p></div>';
        return;
      }
    }
  ...
  _renderTools(container) {
    ...
    html += `<button class="small-btn" onclick="AIAnalysis.bulkAnalyze(false)">🔍 Quick Auto-Tag (${targetLabel})</button>`;
    html += `<button class="small-btn" onclick="AIAnalysis.bulkAnalyze(true)">🧠 Smart AI Analysis (${targetLabel})</button>`;
    html += '<button class="small-btn" onclick="AIAnalysis.cancelAnalysis()">✕ Stop</button>';
    html += '</div>';
    html += '<p class="hint" style="margin-top:4px">Need to connect keys? Open <strong>Settings</strong> in the top bar to add your Abacus.AI key (and other platforms too).</p>';
    html += '</div>';
