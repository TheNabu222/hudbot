...
const QuestTracker = {
  ...
  addQuest(name) {
    ...
    const quest = {
      ...
      description: '', icon: '📜',
      milestones: [{ id: Utils.uid(), text: 'Start', completed: false, branchFlag: '', socialRepDelta: 0, npcId: '' }],
      completionFlagName: '',
  ...
  addMilestone(questId, text) {
    ...
    if (!q) return;
    q.milestones.push({ id: Utils.uid(), text: text || 'New milestone', completed: false, branchFlag: '', socialRepDelta: 0, npcId: '' });
    State.autoSave();
  },
  ...
  advanceMilestone(runtimeQuests, questId, milestoneId, runtimeFlags, runtimeRep) {
    ...
    const ms = (questDef?.milestones || []).find(m => m.id === milestoneId);
    if (ms && (ms.branchFlag === 'boundary-setting' || ms.branchFlag === 'conflict-resolving') && ms.npcId && runtimeRep && runtimeRep[ms.npcId]) {
      runtimeRep[ms.npcId].Friendship = Math.max(0, Math.min(100, Number(runtimeRep[ms.npcId].Friendship || 0) + Number(ms.socialRepDelta || 0)));
    }
  ...
  render(container) {
            ...
            <div class="rpg-quest-card" data-id="${q.id}" style="background:var(--bg-tertiary);border:1px solid var(--border);border-radius:6px;padding:8px;margin-bottom:6px">
              ...
              <div class="quest-milestones">
                ${q.milestones.map((m, mi) => `
                  <div style="display:grid;grid-template-columns:22px 1fr 105px 70px 105px auto;align-items:center;gap:3px;margin-bottom:2px" data-mi="${m.id}">
                    <span style="font-size:10px;color:var(--accent)">${mi + 1}.</span>
                    <input type="text" class="ms-text" value="${this._esc(m.text)}" style="font-size:10px" />
                    <select class="ms-branch" style="font-size:10px">
                      <option value="">No Branch</option>
                      <option value="boundary-setting" ${m.branchFlag === 'boundary-setting' ? 'selected' : ''}>Boundary</option>
                      <option value="conflict-resolving" ${m.branchFlag === 'conflict-resolving' ? 'selected' : ''}>Conflict Resolve</option>
                    </select>
                    <input type="number" class="ms-rep" value="${Number(m.socialRepDelta || 0)}" style="font-size:10px" title="Friendship delta" />
                    <select class="ms-npc" style="font-size:10px">
                      <option value="">NPC</option>
                      ${npcs.map(n => `<option value="${n.id}" ${m.npcId === n.id ? 'selected' : ''}>${this._esc(n.name)}</option>`).join('')}
                    </select>
    ...
    container.querySelectorAll('.rpg-quest-card').forEach(card => {
      ...
      card.querySelectorAll('[data-mi]').forEach(mrow => {
        ...
        mrow.querySelector('.ms-branch')?.addEventListener('change', e => { milestone.branchFlag = e.target.value || ''; State.autoSave(); });
        mrow.querySelector('.ms-rep')?.addEventListener('change', e => { milestone.socialRepDelta = Number(e.target.value || 0); State.autoSave(); });
        mrow.querySelector('.ms-npc')?.addEventListener('change', e => { milestone.npcId = e.target.value || ''; State.autoSave(); });
...
if (typeof globalThis !== 'undefined') globalThis.QuestTracker = QuestTracker;
